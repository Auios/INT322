...
if($_GET)
{
    if($code)
    {
        $isValid = preg_match($pattern1,$code);
        if(!$isValid){$isValid = preg_match($pattern2,$code);}
        if(!$isValid){$isValid = preg_match($pattern3,$code);}
        if(!$isValid){$isValid = preg_match($pattern4,$code);}
        if(!$isValid){$isValid = preg_match($pattern5,$code);}
        if(!$isValid){$isValid = preg_match($pattern6,$code);}
        if(!$isValid){$isValid = preg_match($pattern7,$code);}
    }
}
?>
...
<html>
    ...
    <body>
        ...
        ?>
        <p><a href="lab3-5.php">Previous Lab</a> --- <a href="lab3-6.php">Next Lab</a></p>
    </body>
</html>
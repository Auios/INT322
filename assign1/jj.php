...
for($x=0; $x < count($phone); $x++){
	$sql_query = 'INSERT INTO phone set model="' .trim($phone[$x]). '", os="' .trim($os[$x]). '", model="' .trim($model[$x]). '", price="' .trim($price[$x]). '"';
	mysqli_query($link, $sql_query) or die('query failed'. mysqli_error($link));
}
//validating
if($_POST){
...
if($_POST && $dataValid){
	$min = $_POST['min'];
	$max = $_POST['max'];
	$sql_query = 'SELECT * FROM phone WHERE (model="' .$_POST[phone]. '" && price BETWEEN "'.$min.'" AND "'.$max.'") ORDER BY price' ;
	echo $sql_query;
